...
class Step7_FlakySecondarySpec extends TestKit(ActorSystem("Step7FlakySecondarySpec"))
  with ImplicitSender
  with WordSpecLike
  with Matchers
  ...
  "[07-Flaky-Secondary]" must {

    "case 1: Secondary (in isolation) should properly register itself to the provided mediator" in {
      val mediator = TestProbe()
      val secondary = system.actorOf(Replica.props(mediator.ref, Persistence.props(flaky = true)), "case1-secondary")
    ...
    "case 2: Secondary (in isolation) must handle Snapshots" in {
      ...
      val secondary = system.actorOf(Replica.props(mediator.ref, Persistence.props(flaky = true)), "case2-secondary")
    ...
    "case 3: Secondary should drop and immediately ack snapshots with older sequence numbers" in {
      ...
      val mediator = TestProbe()
      val replicator = TestProbe()
      val secondary = system.actorOf(Replica.props(mediator.ref, Persistence.props(flaky = true)), "case3-secondary")
      val client = session(secondary)
    ...
    "case 4: Secondary should drop snapshots with future sequence numbers" in {
      ...
      val mediator = TestProbe()
      val replicator = TestProbe()
      val secondary = system.actorOf(Replica.props(mediator.ref, Persistence.props(flaky = true)), "case4-secondary")
      val client = session(secondary)
